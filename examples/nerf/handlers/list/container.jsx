          ...
          <table className='table table-striped'>
            ...
            <tbody>
              {this.props.users.length ? this.props.users.map((user) => {
                return (
                  <tr key={user.id}>
                    <td><a href={'/user/' + user.id}><img src={user.avatar} width='35' /></a></td>
                    <td><a href={'/user/' + user.id}>{'@' + user.username}</a></td>
                    <td>{user.email}</td>
                    <td>
                      <div className='btn-group'>
                        <a href={'/user/' + user.id} className='btn btn-default'><span className='glyphicon glyphicon-pencil' aria-hidden='true' /> Edit</a>
                        <button onClick={this.deleteUser} data-user-id={user.id} className='btn btn-danger'><span className='glyphicon glyphicon-trash' aria-hidden='true' /> Delete</button>
                      </div>
                    </td>
                  </tr>
                )
